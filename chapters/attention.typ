...
Alcuni delle conseguenze che questo comporta sono:
 ...
 - Il campo ricettivo cresce linearmente con la profondità della rete, e quindi potrebberoessere necessarie reti estremamente profonde;
 - I pesi che sono fissi dopo l'addestramento, non possono quindi adattarsi dinamicamente al contesto globale dell'immagine.
...
Il contenuto delle seguenti sezioni è riassumibile come segue:
 - @sa:ch: descrive un potente ma computazionalmente costoso meccanismo di attenzione, il quale però permette miglioramenti significativi nelle predizioni, andnado a trovare correlazioni tra ogni _pixel_ o _patch_ del contenuto analizzato;
...
Viene quindi enunciato il concetto di _operatore non locale_ ovvero il blocco presentato in @sa che calcola la risposta in una posizione come una media ponderata delle caratteristiche di tutte le posizioni dell'input.
#block([
L'architettura del così chiamato _Non-local block_ è la seguente:
...
Si vuole far notare che in questo caso le tre matrici sono matrici della dimensione di $3 times ("altezza"dot"larghezza")$, questo per far si che le moltiplicazioni matriciali combinino il valore di un _pixel_ con tutti gli altri.

La prima moltiplicazione tra matrici $Q dot K$ rappresenta il calcolo dell'affinità, ovvero calcolare i pesi di attenzione che determinano l'influenza di ogni _pixel_ (la colonna di $K$ presa in considerazione) sul _pixel_ in analizi (la riga di $Q$ presa in analisi).
...
Il prodotto tensoriale $A dot V$ invece va a creare un'aggregazione ponderata delle informazioni, dando quindi luogo alla matrice di attenzione $Y$.
...
Essendo in seguito fatta una somma tensoriale tra l'input e il risultato del calcolo dell'attenzione (anche detto _residual_, come menzionato in @resnet), il $gamma$ può anche partire da 0, andando quindi inizialmente a non applicare attenzione, questo per stabilizzare inizialmente il _training_ del modello.


#block([
== _Convolutional Block Attention Module_ <cbam:ch>
Il _Convolutional Block Attention Module_ (_CBAM_), presentato in @CBAM è un blocco fortemente basato sulle convoluzioni, che combina la _channel_ e la _spacial attention_, per migliorare le predizioni del modello.
],breakable: false,width: 100%)
#block([
...
#block([
L'architettura del blocco _CBAM_ è quindi la seguente:
...
Il modulo di attenzione sui canali, opera sulla spazialità del tensore (altezza e larghezza) mediante _pooling_ massimo e _pooling_ medio, andando quindi a creare due vettori, i quali rappresenteranno quindi il valore massimo e la media di ciascun canale del tensore di _input_.

#linebreak()

Successivamente entrambi questi vettori vengono passati attraverso un _multi layer perceptron_ da tre strati, con il primo e l'ultimo dalle medesime dimensioni dell'input, mentre quello centrale, essendo più piccolo, applica un fattore di compressione.
...
Il modulo di attenzione spaziale invece, opera sulla profondità del tensore (i canali) mediante _pooling_ massimo e _pooling_ medio, andando quindi a generare due matrici, i quali rappresenteranno i valori massimi e i valori medi sui canali, per ciascun elemento nelle coordinate dell'altezza e della larghezza.

#linebreak()

Queste matrici vengono concatenate sulla dimensione dei canali e successivamente passate attraverso una convoluzione con @kernel di dimensione $3times 3$. Infine l'output di questa operazione viene passato attraverso una sigmoide che genererà quindi la matrice di attenzione sulla spazialità.
