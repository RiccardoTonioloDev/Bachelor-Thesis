...
Si può notare che dopo l'uso di ogni @xinet, è presente una convoluzione trasposta, questo perchè come già discusso nel capitolo #link(<ch:xinet>)[XiNet], questo di per se fa una riduzione della dimensionalità spaziale (altezza e larghezza) all'inizio, e poi ne fa una per ogni coppia all'interno della rete.

La convoluzione trasposta serve per far si che la dimensionalità spaziale sia la medesima che avrebbe prodotto l'@encoder originale di @PyDNetV1.
Anche in questo caso, come per @PyDNetV1, la convoluzione trasposta è seguita da una funzione di attivazione _ReLU_.
...
- L'uso di @xinet come impatta il numero di parametri e il tempo di inferenza?
 - Si vuole far notare che per rispondere a questa domanda, rispetto alle tabelle di valutazione precedenti, sono stati introdotte due nuove metriche di valutazione: numero di parametri ($\#$p), e tempo di inferenza in secondi (Inf. (s)) (il quale viene calcolato su una media del tempo di inferenza su 10 immagini, passate in successione al modello e non in batch, con una elaborazione su _CPU_).

#block([
L'allenamento dei due modelli ha portato ai seguenti risultati:
#ext_eval_table(
  (
    (name: [@PyDNetV1], vals: (1971624.0,0.15,0.16,1.52,6.229,0.253,0.782,0.916,0.964)),
    (name: [@PyDNetV2], vals: (716680.0,0.10,0.157,1.487,6.167,0.254,0.783,0.917,0.964)),
...
Da come si può osservare, sebbbene il numero di parametri sia stato intorno a quelli di @PyDNetV2, se non minore, tutte le altre metriche sono fortemente peggiorate, andando a suggerire che forse non è quello il migliore uso di @xinet all'interno di una rete.
...
Per quanto scritto in @xinet, le reti proposte all'interno del paper sono composte da almeno cinque _XiConv_. Questo suggerisce come la profondità sia un elemento essenziale per il successo nel suo utilizzo.
...
Come si può notare, tutte le architetture della famiglia $beta$ hanno gli @encoder in parallelo. Questo è stato fatto per due motivi:
 ...
 + Permettere agli @encoder composti da @xinet, di diventare più profondi. Questo perchè come spiegato nel capitolo #link(<ch:xinet>)[XiNet], allungando la rete, per ogni coppia successiva di blocchi _XiConv_ si dimezza la dimensionalità spaziale (altezza e larghezza). Non dipendendo ogni livello da quello precedente, possiamo sfruttare questa caratteristica per far si che ogni nuovo encoder si occupi del ridimensionamento della risoluzione del proprio livello.



Il modello $beta" I"$ si ispira ad $alpha" I"$, andando però semplicemente a parallelizzare gli @encoder per quindi permettere delle _XiNet_ più lunghe successivamente.



Il modello $beta" II"$ rispetto a $beta" I"$ cerca di verificare se un eventuale problema potrebbe essere l'uso della convoluzione trasposta, come metodo per aggiustare la risoluzione del tensore. Per verificare ciò le _XiNet_ sono di un blocco più corte rispetto a $beta" I"$, e per far si che si ottengano lo stesso numero di canali di prima, si usa una convoluzione con @kernel di dimensione $1times 1$.



Il modello $beta" III"$ e $beta" IV"$ vanno semplicemente ad aggiungere un livello in più nella piramide, rispetto ai corrispondenti $beta" I"$ e $beta" II"$, così da verificare che eventualmente a parità di numero di livelli si riescano ad ottenere _performance_ simili, se non migliori, a quelle di _PyDNet V2_.

Le due reti però utilizzano approcci differenti:
 - $beta" III"$ aggiunge un @encoder tradizionale di _PyDNet V1_ che è posto in serie con l'@encoder del livello superiore, questo per riuscire ad avere un risparmio sul numero di parametri (poichè avere una _XiNet_ profonda sei blocchi _XiConv_ aumenta significatamente il numero di questi);
 - $beta" IV"$ invece aggiunge una _XiConv_ ancora di un blocco più lunga rispetto al suo livello precedente.
...
#ext_eval_table(
  ...
  2,
  [@PyDNetV1 e @PyDNetV2 vs. PyXiNet $beta$],
)
],breakable: false,width: 100%)
...
Tuttavia notiamo che sebbene il numero di parametri di $beta" III"$ non è minore di quelli di _PyDNet V2_, sono di un $tilde 37%$ inferiori a quelli di _PyDNet V1_ e il tempo di inferenza di questi due modelli è molto simile.
...
Nel tenativo di implementazione di una _self attention_ meno impattante computazionalmente parlando, rispetto al normale modulo di attenzione presentato in @sa, ho realizzato un blocco chiamato _Light Self Attetion Module_ (_LSAM_) che rispetto al _Self Attention Module_ (SAM) discusso precedentemente, applica dell'interpolazione ad _area_ per ridimensionare la risoluzione del tensore in ingresso.
...
È stata utilizzata la normalizzazione dei layer, come consigliato in @layernorm, per ottenere migliori risultati nell'utilizzo della _self attention_.
...
#block([
I risultati ottenuti per la famiglia $MM$ sono quanto riportato in seguito:
#ext_eval_table(
  (
    (name: [@PyDNetV1], vals: (1971624.0,0.15,0.16,1.52,6.229,0.253,0.782,0.916,0.964)),
    (name: [@PyDNetV2], vals: (716680.0,0.10,0.157,1.487,6.167,0.254,0.783,0.917,0.964)),
  ...
  3,
  [@PyDNetV1, @PyDNetV2 e PyXiNet $beta" IV"$  vs. PyXiNet $MM$],
)
],breakable: false,width: 100%)
...
Si può altresì osservera che il tempo di inferenza è tuttavia almeno raddoppiato rispetto a $beta" IV"$ per tutti i modelli.
...
#block([
== PyXiNet *$beta$*CBAM
Come discusso nel capitolo _#link(<ch:attention>)[Attention]_ nella @cbam:ch, uno dei vantaggi principali del blocco _CBAM_ è il suo efficiente uso di risorse garantendo comunque un buon miglioramento delle performance.
],breakable: false,width: 100%)
...
Questa informazione ha quindi guidato il design di un nuovo @decoder, chiamato per l'appunto _CBAM Decoder_ (_CBAMD_). È stato scelto il @decoder come posto dove applicare più volte il blocco, poichè rispecchia l'architettura di una rete neurale convoluzionale a collo di bottiglia.

#block([
L'architettura del nuovo @decoder è come segue:
#figure(image("../images/architectures/Attention-cbamd.drawio.png",width:350pt),caption: [Architettura del @decoder _CBAMD_])
],breakable: false,width: 100%)
...
Si può notare che il blocco _CBAMD_ è stato utilizzato come @decoder, solo del primo livello. Questa scelta è stata fatta per non aumentare di troppo i parametri e il tempo di inferenza, facendolo concentrare solamente sulla risoluzione maggiore (la risoluzione che poi verrà effettivamente visualizzata in un contesto reale).
...
#ext_eval_table(
  ...
  2,
  [@PyDNetV1 e @PyDNetV2 vs. PyXiNet $beta$CBAM],
  res: 102pt
)
],breakable: false,width: 100%)
...
Il modello migliore tra i due, $beta" CBAM I"$ riesce addirittura a stare sotto i 0.20s come tempo di inferenza medio (permettendo un _framerate_ di circa 5 _fps_) e con un numero di parametri inferiore di un $tilde 37%$.


Un'osservazione interessante che può essere fatta è che rispetto alle controparti della famiglia $beta$, l'aggiunta dei vari blocchi _CBAM_ ha contribuito al massimo ad un incremento del $tilde 0,4%$ sul numero dei parametri.

=== _CBAM PyDNet_

Visti gli enormi progessi che il blocco _CBAM_ ci permette di ottenere, sorge spontanea la messa in question dell'utilità dell'@encoder composto dal blocco _XiNet_.

Per verificare questa ipotesi è stata presa l'archiettura originale di _PyDNet V2_, al fine di verificare se posizionando un blocco _CBAM_ prima di ogni convoluzione della rete, con passaggio dei residuali dall'output della convoluzione precedente, si sarebbero potute ottenere performance interessanti.

#block([
Il @decoder quindi rimande il blocco _CBAMD_, che però verrà usato ora su tutti i livelli, mentre l'@encoder (_CBAME_) sarà come segue:
#figure(image("../images/architectures/Attention-cbame.drawio.png",width:200pt),caption: [Architettura dell'@encoder _CBAME_])
],breakable: false,width: 100%)
...
#ext_eval_table(
  ...
  2,
  [@PyDNetV1 e @PyDNetV2 vs. _CBAM PyDNet_],
  res: 102pt
)
],breakable: false,width: 100%)
Per quanto si può notare, posizionare il blocco _CBAM_ dopo ogni convoluzione non fa altro che degradare pesantemente le prestazioni del modello sulla maggior parte delle metriche di valutazione. Si deduce quindi che l'uso di _XiNet_ è sicuramente responsabile per parte del miglioramento delle predizioni del modello.
